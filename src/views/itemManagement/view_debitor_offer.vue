   ...
   <div>
    ...
    />-->
      ...
      <div class="btn-align">
         <h4>All Offers</h4>
         <CButton
            class="bg-primary text-white ml-3 mb-4"
            @click="createNewOfferItems"
         >
         ...
         <table class="table">
            ...
            <tbody>
               <tr v-for="item in datas" :key="item.id">
                  <td>{{ item.id }}</td>
                  <td>
                     {{ item.debitor.id }} : {{ item.debitor.handle }} -
                     {{ item.debitor.name }}
                  </td>
                  ...
                  <td v-if="item.sent">Sent</td>
                  <td v-else-if="item.accepted">accepted</td>
                  <td v-else-if="item.rejected">rejected</td>
                  <td v-else-if="item.solved">solved</td>
                  <td v-else> - </td>
                  ...
                  <td class="p-0 align-middle">
                     ...
                     >
                        <button
                           ...
                           @click="manageOffer(item.id, item.debitor_id)"
                        >
         ...
         this.axios
            ...
            .then((res) => {
               if (res.status == 200) {
                  ...
                  const debitor_id = this.$route.params.debitor_id;
                  const finaldata = [];
                  for (let i = 0; i < this.datas.length; i++) {
                     if (this.datas[i].debitor_id == debitor_id) {
                        finaldata.push(this.datas[i]);
                     }
                  }
                  console.log("=============");
